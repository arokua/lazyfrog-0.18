...
test('a failed mission still yields a row, distinguishable from a win', () => {
	const snapshot = telemetry.buildMissionSnapshot({
		postId: 't3_dead',
		missionMetadata: { encounters: [{ type: 'enemy', enemies: [1, 2, 3] }] },
		navigationStartedMs: 1000,
		playStartedMs: 2000,
	});
	const failed = telemetry.buildTelemetryRow({
		snapshot,
		completedAtMs: 5000,
		outcome: 'failed',
		completionSource: 'out-of-lives-battle',
		config: { telemetryBuild: 'R' },
	});
	assert.strictEqual(failed.outcome, 'failed');
	assert.strictEqual(failed.build, 'R');
	// The clocks must still be populated: how far a run got before dying is the
	// whole point of keeping the row.
	assert.strictEqual(failed.playMs, 3000);
	assert.strictEqual(failed.wallMs, 4000);
	assert.strictEqual(failed.enemyCount, 3);

	const cleared = telemetry.buildTelemetryRow({ snapshot, completedAtMs: 5000, config: {} });
	assert.strictEqual(cleared.outcome, 'cleared');
});
