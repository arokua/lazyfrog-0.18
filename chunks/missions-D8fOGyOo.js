...
const Bug = createLucideIcon("bug", __iconNode$6);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const BUILD_TIME = "2025-11-06T08:16:56.948Z";
function getTimeSinceBuild() {
...
const STORAGE_KEYS = {
  ...
  ESSENCE_LOOT_DICTIONARY: "essenceLootDictionary",
  // Reddit posts tagged as not missions — excluded from sync/re-add
...
function remapFoodEssenceMap(rawEssences, essences) {
  ...
  for (const [rawId, rawQty] of Object.entries(rawEssences || {})) {
    ...
    if (!essences[canonicalId]) {
      essences[canonicalId] = {
        displayName: canonicalId.startsWith("Essence") ? String(rawId).trim() : String(rawId).trim()
      };
    }
...
async function getEssenceLootDictionary() {
  return new Promise((resolve) => {
    chrome.storage.local.get([STORAGE_KEYS.ESSENCE_LOOT_DICTIONARY], (result) => {
      const stored = result[STORAGE_KEYS.ESSENCE_LOOT_DICTIONARY];
      ...
      if (needsPersist) {
        chrome.storage.local.set({ [STORAGE_KEYS.ESSENCE_LOOT_DICTIONARY]: normalized });
      }
      resolve(normalized);
    });
  });
}
async function setEssenceLootDictionary(dictionary) {
  return new Promise((resolve, reject) => {
    const normalized = normalizeEssenceLootDictionary(dictionary, { seedDefaultEssences: false });
    chrome.storage.local.set({ [STORAGE_KEYS.ESSENCE_LOOT_DICTIONARY]: normalized }, () => {
...
function getMissionSearchText(mission) {
  return normalizeEssenceLootText(`${mission?.missionTitle || ""} ${mission?.foodName || ""}`);
}
function countFoodsForEssence(dictionary, essenceId) {
  return Object.values(dictionary?.foods || {}).filter((food) => food?.essences && Object.prototype.hasOwnProperty.call(food.essences, essenceId)).length;
}
function getEssenceFilterOptions(dictionary) {
  const dict = normalizeEssenceLootDictionary(dictionary, { seedDefaultEssences: false });
  return Object.entries(dict.essences || {}).map(([id, entry]) => ({
    id,
    label: entry.displayName || id,
    foodsCount: countFoodsForEssence(dict, id)
...
function missionMatchesEssenceFilter(mission, targetEssences, dictionary) {
  ...
  const haystack = getMissionSearchText(mission);
  if (!haystack) {
    return false;
  }
  const dict = normalizeEssenceLootDictionary(dictionary, { seedDefaultEssences: false });
...
function parseEssenceLootDictionaryImport(jsonText) {
  const parsed = JSON.parse(jsonText);
  return normalizeEssenceLootDictionary(parsed, { seedDefaultEssences: false });
}
function parseFoodNameLines(text) {
  return normalizeFoodNameList(text);
}
...
async function setMissionDisabled(postId, disabled) {
  ...
  if (disabled) {
    if (!userProgress2.disabled.includes(postId)) {
      userProgress2.disabled.push(postId);
    }
  } else {
    const index = userProgress2.disabled.indexOf(postId);
    if (index > -1) {
      userProgress2.disabled.splice(index, 1);
    }
  }
  multiUserData[username] = userProgress2;
  await setMultiUserProgress(multiUserData);
}
async function setMissionsClearedStatus(postIds, cleared) {
  const unique = [...new Set(postIds.map(normalizePostIdFull).filter(Boolean))];
  if (!unique.length) {
    return { updated: 0, markedCleared: 0, markedUncleared: 0 };
  }
  const username = await getCurrentRedditUser();
  const multiUserData = await getMultiUserProgress();
  const progress = multiUserData[username] || createEmptyProgressData();
  progress.cleared = progress.cleared || [];
  progress.clearedAt = progress.clearedAt || {};
  const clearedSet = new Set(progress.cleared);
  let markedCleared = 0;
  let markedUncleared = 0;
  for (const postId of unique) {
    if (cleared) {
      if (!clearedSet.has(postId)) {
        progress.cleared.push(postId);
        clearedSet.add(postId);
        markedCleared++;
      }
      progress.clearedAt[postId] = {
        timestamp: Date.now(),
        duration: void 0
      };
    } else if (clearedSet.has(postId)) {
      progress.cleared = progress.cleared.filter((id) => id !== postId);
      clearedSet.delete(postId);
      markedUncleared++;
      if (postId in progress.clearedAt) {
        delete progress.clearedAt[postId];
      }
    }
  }
  ...
  return {
    updated: cleared ? markedCleared : markedUncleared,
    markedCleared,
    markedUncleared
  };
}
...
function isValidUserProgressData(data) {
  if (!data || typeof data !== "object" || !Array.isArray(data.cleared) || !Array.isArray(data.disabled) || typeof data.clearedAt !== "object" || typeof data.loot !== "object") {
    return false;
  }
  for (const [postId, clearData] of Object.entries(data.clearedAt)) {
    if (typeof clearData !== "object" || clearData === null || typeof clearData.timestamp !== "number") {
      return false;
    }
  }
  return true;
...
async function saveMissionsBatch(missions2) {
  const nonMissionSet = await getNonMissionPostIdSet();
  const toSave = (missions2 || []).filter((mission) => {
    if (!mission?.postId) return false;
    const full = normalizePostIdFull(mission.postId);
    return !nonMissionSet.has(full) && !nonMissionSet.has(String(mission.postId).replace(/^t3_/, ""));
  });
  return new Promise((resolve, reject) => {
...
function getMissionQueueMaxAgeCutoffMs(asOfMs = Date.now()) {
  return asOfMs - MISSION_QUEUE_MAX_AGE_DAYS * 24 * 60 * 60 * 1e3;
}
function isMissionWithinQueueMaxAge(mission, asOfMs = Date.now()) {
  ...
  return postedMs >= getMissionQueueMaxAgeCutoffMs(asOfMs);
}
function isPlaceholderLevelRange(mission) {
  return mission?.minLevel === 1 && mission?.maxLevel === 999;
}
function missionLevelRangeOverlapsFilter(mission, minLevelFilter, maxLevelFilter) {
  const mMin = mission.minLevel;
  const mMax = mission.maxLevel;
  if (mMin == null && mMax == null) return false;
  const filterMin = minLevelFilter ?? 1;
  const filterMax = maxLevelFilter ?? 999;
  if (mMin != null && mMax != null) {
    return mMin <= filterMax && mMax >= filterMin;
  }
  if (mMin != null) {
    return mMin >= filterMin && mMin <= filterMax;
  }
  if (mMax != null) {
    return mMax >= filterMin && mMax <= filterMax;
  }
  return false;
}
...
function normalizeAutomationFilters(filters) {
  ...
  return {
    stars: normalizeStarFilter(source.stars ?? DEFAULT_AUTOMATION_FILTERS.stars),
    minLevel: source.minLevel !== void 0 ? Number(source.minLevel) : DEFAULT_AUTOMATION_FILTERS.minLevel,
    maxLevel: source.maxLevel !== void 0 ? Number(source.maxLevel) : DEFAULT_AUTOMATION_FILTERS.maxLevel,
    targetEssences: Array.isArray(source.targetEssences) ? source.targetEssences : []
  };
}
function missionMatchesBotFilters(mission, normalizedFilters, dictionary) {
  ...
  if (starFilter.length > 0 && !allStarsSelected) {
    const missionDifficulty = getMissionStarDifficulty(mission);
    if (missionDifficulty > 0 && !starFilter.includes(missionDifficulty)) {
      return false;
    }
  }
  const minF = normalizedFilters.minLevel ?? DEFAULT_AUTOMATION_FILTERS.minLevel;
  const maxF = normalizedFilters.maxLevel ?? DEFAULT_AUTOMATION_FILTERS.maxLevel;
  if (!missionLevelRangeOverlapsFilter(mission, minF, maxF)) {
    return false;
  }
  if (normalizedFilters.targetEssences?.length > 0) {
    if (!missionMatchesEssenceFilter(mission, normalizedFilters.targetEssences, dictionary)) {
      return false;
    }
  }
  return true;
}
async function getAutomationFilters() {
  return new Promise((resolve) => {
    chrome.storage.local.get([STORAGE_KEYS.AUTOMATION_FILTERS], (result) => {
      if (chrome.runtime.lastError) {
        resolve(normalizeAutomationFilters(DEFAULT_AUTOMATION_FILTERS));
        return;
      }
      resolve(normalizeAutomationFilters(result[STORAGE_KEYS.AUTOMATION_FILTERS] || DEFAULT_AUTOMATION_FILTERS));
    });
  });
}
async function getFilteredUnclearedMissions(filters, options = {}) {
  const normalizedFilters = filters ? normalizeAutomationFilters(filters) : await getAutomationFilters();
  const applyAgeLimit = options.applyAgeLimit !== false;
  const [missions2, progress, dictionary, nonMissionSet] = await Promise.all([
    getAllMissions(),
    getAllUserProgress(),
    getEssenceLootDictionary(),
    getNonMissionPostIdSet()
  ]);
  const unclearedMissions = Object.values(missions2).filter((m) => {
    ...
    if (applyAgeLimit && !isMissionWithinQueueMaxAge(m)) {
      return false;
    }
    return missionMatchesBotFilters(m, normalizedFilters, dictionary);
  });
  unclearedMissions.sort((a, b) => (b.timestamp || b.postedAt || 0) - (a.timestamp || a.postedAt || 0));
...
async function getNextUnclearedMission(filters) {
  ...
  const excludePostIds = filters?.excludePostIds || [];
  const filteredMissions = excludePostIds.length ? unclearedMissions.filter((m) => !excludePostIds.includes(m.postId)) : unclearedMissions;
  return filteredMissions[0] || null;
}
const BOT_QUEUE_SNAPSHOT_KEY = "lazyfrogBotQueueSnapshot";
async function persistBotQueueSnapshot(snapshot) {
  const payload = {
    ...snapshot,
    refreshedAt: Date.now()
  };
  await new Promise((resolve, reject) => {
    chrome.storage.local.set({ [BOT_QUEUE_SNAPSHOT_KEY]: payload }, () => {
      if (chrome.runtime.lastError) {
        reject(chrome.runtime.lastError);
        return;
      }
  ...
  chrome.runtime.sendMessage({ type: "BOT_QUEUE_UPDATED", snapshot: payload }).catch(() => {
  });
  return payload;
}
async function getBotQueueSnapshot(filters) {
  const normalizedFilters = filters ? normalizeAutomationFilters(filters) : await getAutomationFilters();
  const queue = await getFilteredUnclearedMissions(normalizedFilters);
  return persistBotQueueSnapshot({
...
async function getMissionStats() {
  const [missions, progress, currentFilters] = await Promise.all([
    getAllMissions(),
    getAllUserProgress(),
  ...
  const todayCleared = progress.cleared.filter((postId) => {
    const entry = progress.clearedAt?.[postId];
    const clearedMs = typeof entry === "number" ? entry : entry?.timestamp;
    return typeof clearedMs === "number" && clearedMs > oneDayAgo;
  }).length;
  const uncleared = Object.values(missions).filter(
    (m) => !progress.cleared.includes(m.postId) && !progress.disabled.includes(m.postId)
  ).length;
  const queued = (await getFilteredUnclearedMissions(currentFilters)).length;
  return {
    queued,
...
async function getNextMissions(count, filters) {
  const queue = await getFilteredUnclearedMissions(normalizeAutomationFilters(filters));
  return queue.slice(0, Math.max(0, count || 0));
}
function getArchiveCutoffMs(days = 30) {
  return Date.now() - days * 24 * 60 * 60 * 1e3;
}
function normalizePostIdFull(postId) {
  if (!postId || typeof postId !== "string") return null;
  return postId.startsWith("t3_") ? postId : `t3_${postId}`;
}
function getMissionPostedMsFromFields(mission) {
...
async function fetchRedditPostedMsBatch(postIds) {
  const map = /* @__PURE__ */ new Map();
  const unique = [...new Set(postIds.map(normalizePostIdFull).filter(Boolean))];
  for (let i = 0; i < unique.length; i += 100) {
    const chunk = unique.slice(i, i + 100);
    const ids = chunk.join(",");
    const response = await fetch(`https://www.reddit.com/api/info.json?id=${ids}`, {
      credentials: "include",
      headers: {
        Accept: "application/json",
        "User-Agent": "LazyFrog/0.16.1 (Chrome Extension; mission cleanup)"
      }
    });
    if (!response.ok) {
      throw new Error(`Reddit API info.json failed: ${response.status}`);
    }
    const data = await response.json();
    for (const child of data?.data?.children || []) {
      const post = child?.data;
      if (!post?.id) continue;
      const postedMs = (post.created_utc || post.created || 0) * 1e3;
      if (!postedMs) continue;
      map.set(`t3_${post.id}`, postedMs);
      map.set(post.id, postedMs);
    }
...
async function pruneUserProgressPostIds(postIds) {
  if (!postIds.length) return 0;
  const removeSet = new Set(postIds);
  const multiUserData = await getMultiUserProgress();
  let pruned = 0;
  for (const username of Object.keys(multiUserData)) {
    const progress = multiUserData[username];
    if (!progress || typeof progress !== "object") continue;
    if (Array.isArray(progress.cleared)) {
      progress.cleared = progress.cleared.filter((id) => !removeSet.has(id));
    }
    if (Array.isArray(progress.disabled)) {
      progress.disabled = progress.disabled.filter((id) => !removeSet.has(id));
    }
    if (progress.clearedAt && typeof progress.clearedAt === "object") {
      for (const id of postIds) {
        if (progress.clearedAt[id]) {
          delete progress.clearedAt[id];
          pruned++;
        }
      }
    }
    if (progress.loot && typeof progress.loot === "object") {
      for (const id of postIds) {
        if (progress.loot[id]) {
          delete progress.loot[id];
        }
      }
    }
    multiUserData[username] = progress;
...
async function persistMissionsObject(missions2) {
  return new Promise((resolve, reject) => {
...
async function getNonMissionPostIds() {
  return new Promise((resolve) => {
    chrome.storage.local.get([STORAGE_KEYS.NON_MISSION_POSTS], (result) => {
      if (chrome.runtime.lastError) {
        resolve([]);
        return;
      }
      const list = result[STORAGE_KEYS.NON_MISSION_POSTS];
      resolve(Array.isArray(list) ? list.filter((id) => typeof id === "string" && id.length > 0) : []);
    });
  });
}
async function getNonMissionPostIdSet() {
  const ids = await getNonMissionPostIds();
  const set = /* @__PURE__ */ new Set();
  for (const id of ids) {
    const full = normalizePostIdFull(id);
    if (!full) continue;
    set.add(full);
...
async function isNonMissionPost(postId) {
  if (!postId) return false;
  const set = await getNonMissionPostIdSet();
  const full = normalizePostIdFull(postId);
  return set.has(full) || set.has(String(postId).replace(/^t3_/, ""));
}
async function setNonMissionPostIds(postIds) {
  const unique = [...new Set((postIds || []).map(normalizePostIdFull).filter(Boolean))];
  return new Promise((resolve, reject) => {
    chrome.storage.local.set({ [STORAGE_KEYS.NON_MISSION_POSTS]: unique }, () => {
      if (chrome.runtime.lastError) {
        reject(chrome.runtime.lastError);
        return;
      }
...
async function addNonMissionPosts(postIds) {
  const existing = await getNonMissionPostIds();
  const set = new Set(existing.map(normalizePostIdFull).filter(Boolean));
  for (const id of postIds || []) {
    const full = normalizePostIdFull(id);
    if (full) set.add(full);
  }
  const next = [...set];
  await setNonMissionPostIds(next);
  return next.length;
}
async function removeNonMissionPosts(postIds) {
  const removeSet = new Set((postIds || []).map(normalizePostIdFull).filter(Boolean));
  if (!removeSet.size) return 0;
  const existing = await getNonMissionPostIds();
  const next = existing.map(normalizePostIdFull).filter((id) => id && !removeSet.has(id));
  await setNonMissionPostIds(next);
  return removeSet.size;
}
async function deleteMissions(postIds, options = {}) {
  const tagAsNonMission = options.tagAsNonMission === true;
  const ids = [...new Set((postIds || []).map(normalizePostIdFull).filter(Boolean))];
  if (!ids.length) {
    return { deleted: 0, tagged: 0 };
  }
  const missions2 = await getAllMissions();
  let deleted = 0;
  const removedIds = [];
  ...
  if (removedIds.length) {
    ...
    await pruneUserProgressPostIds(removedIds);
  }
  let tagged = 0;
  if (tagAsNonMission) {
    await addNonMissionPosts(ids);
    tagged = ids.length;
  }
...
async function pruneUnflairedJunkMissions(options = {}) {
  ...
  const result = await deleteMissions(unique, { tagAsNonMission });
  return { removed: result.deleted, tagged: result.tagged };
}
async function cleanupArchivedMissions(days = 30) {
  const cutoffMs = getArchiveCutoffMs(days);
  const missions2 = await getAllMissions();
  const toDelete = /* @__PURE__ */ new Set();
  const needsLookup = [];
  let enrichedPostedAt = 0;
  for (const [postId, mission] of Object.entries(missions2)) {
    let postedMs = getMissionPostedMsFromFields(mission);
    if (!postedMs && typeof mission?.timestamp === "number" && mission.timestamp > 0 && mission.timestamp <= cutoffMs) {
      postedMs = mission.timestamp;
    }
    if (postedMs) {
      if (postedMs < cutoffMs) {
        toDelete.add(postId);
      }
      continue;
    }
    needsLookup.push(postId);
  }
  let lookupFailed = null;
  if (needsLookup.length) {
    try {
      const lookedUp = await fetchRedditPostedMsBatch(needsLookup);
      for (const postId of needsLookup) {
        const postedMs = lookedUp.get(postId) || lookedUp.get(normalizePostIdFull(postId));
        if (!postedMs) {
          continue;
        }
        if (!missions2[postId].postedAt) {
          missions2[postId].postedAt = postedMs;
          enrichedPostedAt++;
        }
        if (postedMs < cutoffMs) {
          toDelete.add(postId);
...
const missions = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  getNonMissionPostIds,
  importMissions,
  isNonMissionPost,
  markAllMissionsIncomplete,
  missionLevelRangeOverlapsFilter,
  normalizeAutomationFilters,
  pruneUnflairedJunkMissions,
  removeNonMissionPosts,
  saveMissionsBatch
}, Symbol.toStringTag, { value: "Module" }));
export {
  Bug as B,
  ChevronRight as C,
  DEFAULT_AUTOMATION_FILTERS as D,
  DEFAULT_ESSENCE_LOOT_DICTIONARY as E,
  Play as P,
  Settings as S,
  VERSION as V,
  __vitePreload as _,
  ...
  getAllMissions as f,
  getMission as g,
  getAllUserProgress as h,
  importMissions as i,
  jsxRuntimeExports as j,
  ChartColumn as k,
  Star as l,
  clientExports as m,
  getTimeSinceBuild as n,
  getCurrentRedditUser as o,
  userProgress as p,
  missions as q,
  reactExports as r,
  saveMissionsBatch as s,
  userDetection as u,
  getEssenceLootDictionary as v,
  setEssenceLootDictionary as w,
  missionMatchesEssenceFilter as x,
  parseEssenceLootDictionaryImport as y,
  normalizeEssenceLootDictionary as z,
  countFoodsForEssence as A,
  getEssenceFilterOptions as F,
  getMissionStats as G,
  parseFoodNameLines as H,
  getAutomationFilters as I,
  getFilteredUnclearedMissions as J,
  getBotQueueSnapshot as K,
  getNextUnclearedMission as L,
  normalizeAutomationFilters as M,
  missionLevelRangeOverlapsFilter as N,
  MISSION_QUEUE_MAX_AGE_DAYS as O,
  deleteMissions as Q,
  getNonMissionPostIds as R,
  isNonMissionPost as T,
  addNonMissionPosts as U,
  removeNonMissionPosts as W,
  getNextMissions as X,
  setMissionsClearedStatus as Y
};
